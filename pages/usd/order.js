...
let date=today.getDate() + "-"+ parseInt(today.getMonth()+1) +"-"+today.getFullYear();
  ...
  const changemail = (e) => {
   ...
   axios.post(API_URL+'add.php', {email : e.target.value,date : date , ip:ip,website : "usd-lux"}).then(function(result) {
   } )
...
const initcheck = event => {
   ...
   proxychecked ? myplan = myplan + '' : myplan ;
   ...
   const updatedata = {uniqueid :myuniqueid , placeddate : getCurrentDate(), pack : myplan,ip:ip,  paid : "init checkout " , plan :packageid  ,total : lastprice,currency : currency,mac : mac ,  email : myemail ,wtp : wtp , proxy : proxychecked ? 'yes' : 'no', adult : adult ? 'yes' : 'no' , device : Device, type : type, website : 'usd-lux'};
   if ( subid == null ){
      if ( uniqueid == null) {
        
         axios.post(API_URL+'addSubscription.php', updatedata).then(function(result) {
     
            document.location.href = 'https://luxuryhosting.shop/usd/payment?subid='+myuniqueid
         } )
      }
      else {
     
            document.location.href = '/usd/order'
...
const Month12 = () => {
  ...
  const toggleEur = () => {
   ...
   return (
      ...
      <Script
        src="https://www.googletagmanager.com/gtag/js?id=G-X744XMZG8P"
          ...
          <div className="px-4 py-5 md:hidden">
            ...
            <div className="shadow-md sm:flex items-center sm:space-x-4 sm:space-y-0 space-y-2 bg-white py-4 px-6 rounded-xl max-w-2xl cursor-pointer">
                     <div onClick={ () => toggleProxyCheck() } className="flex-1">
                        <h2 className="uppercase"> ADULT CHANNELS / XXX CHANNELS  <span className="bg-green-400 text-white rounded-full uppercase px-2 ml-1 ">optional</span> </h2>
...
<input 
 ...
 placeholder="00:1A:79:XX:XX:XX" className="appearance-none block w-full bg-grey-lighter text-grey-darker border-2 border-grey-lighter rounded-lg h-10 px-4" type="text" name="mac"  style={{outline: 'none'}} />
                    ...
                    <div class="space-x-4 ">
                           ...
                           ><span className="inline-block flex-1">+ Premium Adult Channels for {packname}
